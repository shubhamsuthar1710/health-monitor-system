...
export async function POST(request) {
  try {
    ...
    console.log('[send-edit-confirmation] Request received:', {
    ...
    if (!resend) {
      console.error('[send-edit-confirmation] RESEND_API_KEY not configured');
      if (process.env.NODE_ENV === 'development') {
        console.log('[send-edit-confirmation] DEV MODE: Skipping email send');
        return NextResponse.json({
          success: true,
    ...
    if (error) {
      console.error('[send-edit-confirmation] Resend API error:', {
        name: error.name || 'N/A',
    ...
    console.log('[send-edit-confirmation] Email sent successfully, ID:', data?.id);
    return NextResponse.json({
      success: true,
    ...
    console.error('[send-edit-confirmation] Unexpected error:', {
      name: error.name || 'N/A',
      message: error.message || 'N/A',
    });
...
function getEmailHtml({ fullName, editUrl }) {
  return `
    <!DOCTYPE html>
    <html>
    <head>
        ...
        <div style="padding: 40px;">
          <div style="background-color: #f8fafc; border: 1px solid #e2e8f0; border-radius: 12px; padding: 32px; margin-bottom: 32px;">
            ...
            <div style="text-align: center; margin: 32px 0;">
              <a href="${editUrl}"
                 style="background: linear-gradient(135deg, #2563eb 0%, #1d4ed8 100%); color: #ffffff; padding: 16px 32px; border-radius: 12px; text-decoration: none; font-weight: 600; font-size: 16px; display: inline-block; box-shadow: 0 4px 6px -1px rgba(37, 99, 235, 0.2);">
                Edit Profile Now
              </a>
            </div>
            <div style="background-color: #fffbeb; border-left: 4px solid #f59e0b; padding: 20px; border-radius: 8px; margin-top: 32px;">
              <p style="font-size: 14px; font-weight: 600; color: #92400e; margin: 0 0 8px 0;">Security Information</p>
          ...
          <div style="background-color: #f4f4f5; padding: 16px; border-radius: 8px;">
            ...
            <p style="font-size: 13px; color: #2563eb; word-break: break-all; margin: 0; font-family: monospace;">${editUrl}</p>
          </div>
        </div>
